...
def reaction_tag_tops(base_path: str, date: str, depth: int, spark: SparkSession) -> DataFrame:
    reaction_paths = input_paths(base_path, date, depth)
...
date = "2022-05-25"
days_count = 7
events_base_path = "/user/sergeykhar/data/events"
output_base_path = "/user/sergeykhar/analytics/user_interests_d2022-05-25"


spark = SparkSession.builder \
    .master("yarn") \
    .appName(f"UserInterestsJob-{date}-{days_count}") \
    .getOrCreate()

df=calculate_user_interests(base_path=events_base_path, date=date, depth=days_count, spark=spark)
